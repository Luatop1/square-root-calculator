...
while True:
    ...
    if d < 0: # Проверка на отрицательность дискриминанта
        print(f"Данная программа пока что не может решить уравнение с отрицательным дискриминантом, Дискриминант = {d}")
    elif d == 0: # Проверка на 0 дискриминанта
        x = (-b) / (2 * a)
        print(d)
    else:
        x1 = (-b + math.sqrt(d)) / (2 * a) # Находит первый корень
        x2 = (-b - math.sqrt(d)) / (2 * a) # Находит второй корень
        print(f"x¹ = {x2}, x² = {x1}")

    while True:
        ask_solution = input("Нужно пошагавое решение? [y/n]")
        if ask_solution == "y" and d > 0: # Выводит поэтапное решение уравнения если пользователь написал y
